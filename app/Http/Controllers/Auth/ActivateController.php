...
class ActivateController extends Controller
{
    ...
    public function activate($token)
    {
        if ($this->service->activateUser($token)) {
            return redirect()->away(env('FRONTEND_URL').'/#/login?message=Cuenta activada');
        }

        return redirect()->away(env('FRONTEND_URL') . '/#/login/?message=No hemos podido validar tu cuenta');
    }
}
